...
func TestRegisterHandlers_RateLimiterIntegration(t *testing.T) {
	...
	if _, err := registerHandlers(rtr, proc, collector, limiter, nil, cfg, log); err != nil {
...
func TestRegisterHandlers_StorageMiddlewareIntegration(t *testing.T) {
	...
	if _, err := registerHandlers(rtr, proc, collector, nil, storageMiddleware, cfg, log); err != nil {
		t.Fatalf("registerHandlers() failed: %v", err)
	}
}
...
func TestRegisterHandlers_DisabledMiddleware(t *testing.T) {
	...
	if _, err := registerHandlers(rtr, proc, collector, nil, nil, cfg, log); err != nil {
		t.Fatalf("registerHandlers() failed: %v", err)
	}
}
...
func TestRegisterHandlers_AllMiddleware(t *testing.T) {
	...
	if _, err := registerHandlers(rtr, proc, collector, limiter, storageMiddleware, cfg, log); err != nil {
		t.Fatalf("registerHandlers() failed: %v", err)
	}
}
